...
{
    __shared__ double As[STRIDE][BM];
    ...
    for (int blockidx = 0; blockidx < mid; blockidx += STRIDE)
    {
        for (int i = tid; i < BM * STRIDE; i += blockDim.x * blockDim.y)
        {
            ...
            if(blockIdx.y * BM + row<rows1 && blockidx + col<mid)
                As[col][row] = A[(blockIdx.y * BM + row) * mid + blockidx + col];
        }
        for (int i = tid;
             i < STRIDE * BN;
        ...
        for (int k = 0; k < STRIDE; k++)
        {
            ...
            // {
            //     regM[i] = As[k][threadIdx.y*PERTHREADY + i];
            // }
            double2* As_vec = reinterpret_cast<double2*>(&As[k][threadIdx.y * PERTHREADX]);
            double2 tmp0 = As_vec[0];
            double2 tmp1 = As_vec[1];
            double2 tmp2 = As_vec[2];
            double2 tmp3 = As_vec[3];
            ...
            double2* Bs_vec = reinterpret_cast<double2*>(&Bs[k][threadIdx.x * PERTHREADY]);
            tmp0 = Bs_vec[0];
            tmp1 = Bs_vec[1];
            tmp2 = Bs_vec[2];
            tmp3 = Bs_vec[3];
